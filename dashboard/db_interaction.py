...
def Is_the_server_already_wanted_state(id_machine, wanted_state):
    #stopped/running
    bool_result = ""
...
def all_operations(wanted_state,list_machine):
    ...
    for machine_target in list_machine:
        if not(Is_the_server_already_wanted_state(machine_target, wanted_state)):
            function.action(False,machine_target,operation_to_perform)
            print(str(machine_target)+" is now "+str(wanted_state))
        elif Is_the_server_already_wanted_state(machine_target, wanted_state):
